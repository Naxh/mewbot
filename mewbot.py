...
async def trainer(ctx, user: discord.Member=None):
    tconn = await asyncpg.connect('postgresql://postgres:7890@localhost:5432/trainers')
    if user is None:
        user = ctx.author
...
async def on_message(message):
    ...
    if message.content.startswith(vl):
        ...
        pconn = await asyncpg.connect('postgresql://postgres:7890@localhost:5432/pokemon')
        hpiv = random.randint(1, 31)
        atkiv = random.randint(1, 31)
        defiv = random.randint(1, 31)
        spaiv = random.randint(1, 31)
